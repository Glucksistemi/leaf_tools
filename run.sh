...
DOCKER_OPTS="${DOCKER_PARAMS[$ENVIRONMENT]} ${NETWORK[$ENVIRONMENT]} $VOLUME"
...
eval "$DOCKER_COMMAND $DETACH --name $CONTAINER_NAME $DOCKER_OPTS --tmpfs=/ramdisk --env-file=$CONFIG $IMAGE_NAME $COMMAND"